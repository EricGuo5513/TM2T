...
from motion_loaders.t2m_seq2seq_model_dataset import T2MSeq2SeqGeneratedDataset
...
def get_motion_loader(opt_path, batch_size, ground_truth_dataset, mm_num_samples, mm_num_repeats, device):
    ...
    if 'T2M' in opt.name:
        if "Seq2Seq" in opt.name:
            dataset = T2MSeq2SeqGeneratedDataset(opt, ground_truth_dataset, w_vectorizer, mm_num_samples, mm_num_repeats)
    else:
        raise KeyError('Dataset not recognized!!')
